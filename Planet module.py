...
class Planet:
    ...
    def dist_surf(self, r, ang, t):  # Returns distance to the surface
        r1 = self.coord(t)
        if self.r0[1] is not None:
            return vect_sum(r, ang, r1[0], r1[1])[0] - self.radius
        else:
    ...
    def gravity(self, r, ang, t):  # Returns g, angle to the center of the planet
        ...
        if self.r0[1] is not None:
            return tuple((G * self.m) / R ** 2, vect_sum(r, ang, a, b)[1])
        else:
            return tuple((G * self.m) / R ** 2, ang)
...
Moon = Planet(384467, 0, 7.3477 * 10 ** 22, 1737100, 2 * math.pi / (27 * 24 * 60 * 60))
